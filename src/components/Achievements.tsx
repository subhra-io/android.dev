        ...
        <div className={`transition-all duration-1000 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6 mb-16">
            <div className="text-center">
              ...
              <div className="text-navy-300 text-sm">Published Apps</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-400 mb-2">
                {publishedApps.reduce((sum, app) => sum + parseInt(app.downloads.replace(/[^0-9]/g, '')), 0).toLocaleString()}+
              </div>
              <div className="text-navy-300 text-sm">Total Downloads</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-yellow-400 mb-2">
                {(publishedApps.reduce((sum, app) => sum + app.rating, 0) / publishedApps.length).toFixed(1)}
              </div>
              <div className="text-navy-300 text-sm">Avg Rating</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-cyan-400 mb-2">
                2.5M+
              </div>
              <div className="text-navy-300 text-sm">Daily Transactions</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-400 mb-2">
                {awards.length}
              </div>
              <div className="text-navy-300 text-sm">Awards Won</div>
            </div>
          </div>
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {publishedApps.map((app, index) => (
                <div
                  key={app.id}
                  className={`bg-navy-600/50 rounded-lg p-6 border border-navy-500 hover:border-green-400/50 transition-all duration-500 hover:transform hover:-translate-y-2 ${
                ...
                >
                  <div className="flex items-start space-x-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="text-lg font-semibold text-navy-100">{app.title}</h4>
                        <a
                  ...
                  <div className="border-t border-navy-500 pt-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {awards.map((award, index) => (
                <div
                  key={award.id}
                  className={`bg-gradient-to-r from-yellow-400/10 to-orange-400/10 rounded-lg p-6 border border-yellow-400/20 hover:border-yellow-400/50 transition-all duration-500 hover:transform hover:-translate-y-2 ${
                    inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
                  style={{ transitionDelay: `${index * 150}ms` }}
                >
                  <div className="flex items-start space-x-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="text-lg font-semibold text-navy-100">{award.title}</h4>
                        {award.certificate && (
                          <a
                  ...
                  <div className="space-y-2 text-sm text-navy-400">
                    ...
                    {award.teamSize && (
                      <div className="flex items-center">
                        <Users className="w-4 h-4 mr-2 text-yellow-400" />
                        Team of {award.teamSize} members
                      </div>
                    )}
                  </div>
                </div>
